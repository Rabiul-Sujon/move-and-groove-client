...
export default function Home() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const response = await api.get('/classes?limit=4&sortBy=createdAt&sortOrder=desc');
        setFeaturedClasses(response.data.data.classes || []);
      } catch (err) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            {loading ? (
              ...
              <p className="text-center text-gray-500 col-span-full">No classes available yet.</p>
            ) : (
